...
/**
 * Converts a JSON object into a flattened, readable string for AI context.
 */
const stringifyForIndexing = (parsed: any, original: string): string => {
    if (typeof original === 'string' && original.length > 100) return original; // Prefer original if it's text
    try {
        // A simple stringification that removes JSON syntax for better NLP processing
            ...
            try {
                ...
                for (let i = 1; i <= pdf.numPages; i++) {
                    ...
                    if (textContent.items.length > 10) {
                        ...
                        // Fix: The RenderParameters type definition for pdf.js requires a 'canvas' property.
                        // We provide it here to satisfy the TypeScript compiler.
...
/**
 * A new parser function dedicated to extracting full, unsummarized text for RAG indexing.
 * @param file The file to parse.
 * @returns A promise that resolves to the full text content of the file.
 */
export const extractFullTextFromFile = async (file: File): Promise<string> => {
    ...
    // Return empty for binary or failed parses
    return '';
};